...
describe('Components (non-JSX)', () => {
	...
	class BasicComponent1 extends Component {
		render() {
			return createElement('div', { className: 'basic' }, 
				createElement('span', { className: this.props.name }, 
					'The title is ', 
					this.props.title
				)
			);
	...
	it('should render a basic component with inputs', () => {
		...
		expect(container.firstChild.firstChild.firstChild.nextSibling.getAttribute('type')).to.equal('checkbox');
		expect(container.firstChild.firstChild.tagName).to.equal('DIV');
		expect(container.firstChild.firstChild.innerHTML).to.equal('<label>A label</label><input type="checkbox">The title is abc');
	...
	class BasicComponent1d extends Component {
		render() {
			return createElement('div', { className: 'basic' },
				createElement('label', {},
					createElement('input', { type: 'password', disabled: this.props.isDisabled }),
					'The title is ',
					this.props.title
				)
			);
	...
	describe('should render a basic component with conditional element', () => {
		const tpl3625453295 = function () {
			return element('h1').children('BIG');
		};
		const tpl4021787591 = function () {
			return element('h2').children('small');
		};
	...
	describe('should render a basic stateless component', () => {
		...
		it('should render a stateless component', () => {
			...
			const text = () => {
				return element('span').text('123abc');
			};
		...
		const tpl3578458729 = function (v0) {
			return element('div').props({ className: 'login-view bg-visma' }).children(v0);
		};
		const tpl188998005 = function () {
			return element('div').text('VISIBLE');
		};

		const tpl3754840163 = function (v0) {
			return element('div').children(element('button').events({
				onclick: v0
