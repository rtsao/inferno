...
describe('Children - (non-JSX)', () => {
	...
	preDefined.forEach((arg) => {
		[{
			...
			template: () => element('div').children(arg.value)
		}].forEach((test) => {
			it(test.description, () => {
	...
	preDefined.forEach((arg) => {
		[{
			...
			template: () => element('div').children(element('span').children(arg.value))
		}].forEach((test) => {
			it(test.description, () => {
				render(test.template(), container);
				expect(container.firstChild.nodeType).to.equal(1);
	...
	preDefined.forEach((arg) => {
		[{
			...
			template: () => element('div').children(element('span').children(element('b').children(element('b').children(arg.value))))
		}].forEach((test) => {
			it(test.description, () => {
				render(test.template(), container);
				expect(container.firstChild.nodeType).to.equal(1);
	...
	preDefined.forEach((arg) => {
		[{
			...
			template: (child) => element('div').children(child)
		}].forEach((test) => {
			it(test.description, () => {
	...
	preDefined.forEach((arg) => {
		[{
			...
			template: (child) => element('div').children(element('b').children(child))
		}].forEach((test) => {
			it(test.description, () => {
				render(test.template(arg.value), container);
				expect(container.firstChild.firstChild.nodeType).to.equal(1);
